...
class RecordResults():
    ...
    def train_intent_batch_update(self, itern, data, intent_gt, intent_prob_gt, intent_prob, loss, loss_intent):
        ...
        # Check if intent_prob has elements
        if intent_prob.size > 0:  # Change this line
            # (3.3) intent
            ...
            # Uncomment the assert if you're sure about its purpose
            # assert len(self.intention_gt[0]) == 1  # self.args.predict_length, intent only predict 1 result
        else:
            print(f"Warning: intent_prob is empty for batch {itern}. Skipping update.")  # Added logging
