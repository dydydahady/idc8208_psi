...
def init_db(video_list, db_log, args, test_flag):
    db = {}
#     data_split = 'train' # 'train', 'val', 'test'
    ...
    if args.dataset == 'PSI2.0':
        if test_flag == 'test':
            extended_folder = 'PSI2.0_Test/annotations/cognitive_annotation_extended'
        else:
    ...
    for video_name in sorted(video_list):
        try:
            print("Joined path: " + (os.path.join(dataroot, extended_folder, video_name, 'pedestrian_intent.json')))
